...
void c_ragebot::update_predicted_eye_pos()
{
	...
	const float server_time = HACKS->global_vars->realtime;
	const float simulation_time = HACKS->local->sim_time();
	const float sim_delta = server_time - simulation_time;

	auto max_speed = HACKS->local->is_scoped() ? HACKS->weapon_info->max_speed_alt : HACKS->weapon_info->max_speed;
	auto velocity = unpredicted_vars->velocity;

	static auto sv_friction = HACKS->cvar->find_convar(CXOR("sv_friction"));
	static auto sv_stopspeed = HACKS->cvar->find_convar(CXOR("sv_stopspeed"));
	static auto sv_gravity = HACKS->cvar->find_convar(CXOR("sv_gravity"));

	const float friction_val = sv_friction->get_float();
	const float stop_speed_val = sv_stopspeed->get_float();
	const float gravity_val = sv_gravity->get_float();
	const float interval_per_tick = HACKS->global_vars->interval_per_tick;

	auto speed = std::max<float>(velocity.length_2d(), 1.f);

	int latency_ticks = 0;
	auto net_channel = HACKS->engine->get_net_channel();
	if (net_channel) {
		latency_ticks = TIME_TO_TICKS(net_channel->get_avg_latency(FLOW_INCOMING));
	}

	float sim_time_factor = std::clamp(sim_delta * 0.5f, 0.0f, 0.1f);
	auto dynamic_factor = std::min<float>(speed / max_speed, 1.0f) * 7.0f + sim_time_factor * 60.0f;
	auto max_stop_ticks = std::max<int>(static_cast<int>(dynamic_factor) + latency_ticks, 0);

	auto max_predict_ticks = std::clamp(max_stop_ticks, 0, 16);

	//no need for prediction if we're not moving
	if (max_predict_ticks == 0 || speed < 60.0f)
	{
	...
	for (int i = 0; i < max_predict_ticks; ++i)
	{
		...
		if (speed >= 100.0f)
		{
			const float time = interval_per_tick;

			if (flags.has(FL_ONGROUND))
			{
				float control = (speed < stop_speed_val) ? stop_speed_val : speed;
				float drop = control * friction_val * time;
		...
		if (!flags.has(FL_ONGROUND))
		{
			pred_velocity.z -= gravity_val * 0.5f * interval_per_tick;
		}

		vec3_t next_origin = pred_origin + pred_velocity * interval_per_tick;

		c_game_trace trace;
		c_trace_filter_simple filter(HACKS->local);
		HACKS->engine_trace->trace_ray(
			ray_t(pred_origin, next_origin, HACKS->local->bb_mins(), HACKS->local->bb_maxs()),
			MASK_PLAYERSOLID,
		...
		if (trace.fraction < 1.0f)
		{
			for (int j = 0; j < 3; j++)
			{
				const float dot = pred_velocity.dot(trace.plane.normal);
				if (dot < 0.0f)
				{
					pred_velocity -= trace.plane.normal * dot;

					const float dampening = 0.9f;
					pred_velocity *= dampening;
				}
			}

			next_origin = trace.end;
		}

		game_movement::extrapolate(HACKS->local, next_origin, pred_velocity, flags, flags.has(FL_ONGROUND));

		last_predicted_velocity = pred_velocity;
		last_predicted_origin = next_origin;
	}
...
void pre_cache_centers(int damage, std::vector<int>& hitboxes, vec3_t& predicted_eye_pos, rage_player_t* rage)
{
	...
	auto get_overall_damage = [&](anim_record_t* record) -> int
		{
			...
			// fallback to eye_pos if no results
			if (points.empty())
				points = get_hitbox_points(damage, hitboxes, anim->eye_pos, predicted_eye_pos, rage, record, true);

			int total_damage = 0;
			for (const auto& point : points)
			{
				rage->points_to_scan.push_back(point);
				total_damage += point.damage;
			}
	...
	if (start_fakelag_fix(rage->player, lagcomp))
	{
		...
		get_overall_damage(best); // fk the result, at least we will have the record
	}
	else
	{
		...
		for (auto it = lagcomp->records.begin(); it != lagcomp->records.end(); ++it)
		{
			if (it->valid_lc)
			{
				if (!first) first = &*it;
		...
		if (first && last)
		{
			int first_dmg = get_overall_damage(first);
			int last_dmg = get_overall_damage(last);
			best = (last_dmg > first_dmg) ? last : first;
		}
		else if (first)
		{
			get_overall_damage(first);
			best = first;
		}
	}
